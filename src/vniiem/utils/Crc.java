...
public class Crc {
	...
	private static final int[] table = { 0x0000, 0x1021, 0x2042, 0x3063, 0x4084, 0x50a5, 0x60c6, 0x70e7, 0x8108, 0x9129,
	...
	public static short crc16Ccitt(Byte[] data, int start, int length) {
		int crc = 0xFFFF; // initial value
		int temp = 0;
		for (int i1 = start; i1 < length; i1++) {
			temp = (data[i1] ^ (crc >>> 8)) & 0xFF;
	...
	public static short crc16Ccitt(ByteBuffer data, int start, int length) {
		int crc = 0xFFFF; // initial value
		int temp = 0;
		for (int i1 = start; i1 < length; i1++) {
			temp = ((data.get(i1) & 0xff) ^ (crc >>> 8)) & 0xFF;
			crc = table[temp] ^ (crc << 8);
		}
